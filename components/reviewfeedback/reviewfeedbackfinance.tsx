...
import { postAddFinanceReview } from "../services/api/review-api";
import styles from "./reviewfeedback.module.css";

const ReviewFeedbackFinance = (props: any) => {
  const cookies = new Cookies();
  const [trustRating, setFoodRating] = useState(1);
  const [serviceRating, setServiceRating] = useState(1);
  const [supportRating, setAmbienceRating] = useState(1);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      postAddFinanceReview(reviewData)
